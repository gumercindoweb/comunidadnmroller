...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const CAMPAIGN_ID =
      Deno.env.get('GETRESPONSE_CAMPAIGN_ID_CLASE_GRATIS') ??
    ...
    const res = await fetch('https://api.getresponse.com/v3/contacts', {
      ...
      body: JSON.stringify({
        ...
        dayOfCycle: '0',
        tags: [{ tagId: 'clase-gratis' }].slice(0, 0), // tags require IDs; omitted safely
      }),
    })
    ...
    console.error('subscribe-clase-gratis error', err)
